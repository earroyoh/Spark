...
kubectl apply -f spark-k8s-submit-deployment.yaml -n spark
kubectl expose deployment spark-job --port=4040 --type=LoadBalancer --external-ip=$EXTERNAL_IP --name=spark-jupyter -n spark
